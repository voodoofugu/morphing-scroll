 ...
 *
 * Нужна одному `scrollToObject`: по имени группы он едет к её первому
 * объекту. Отдельного пропа под группы нет — ключ и так обязан быть
 * уникальным, и дописать к нему имя в скобках дешевле, чем вести рядом
 * второй список. `"news-3[news]"` — объект `news-3` из группы `news`.
 */
