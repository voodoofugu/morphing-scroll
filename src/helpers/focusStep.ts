...
const intoViewDelta = (view: DOMRect, box: DOMRect, pads: Pads) => {
  const along = (
    ...
    const behind = boxStart - viewStart; // сколько до объекта позади
    const ahead = boxEnd - viewEnd; // насколько объект вылез вперёд

    /*
     * Доехать «ровно до края» мало: объект встаёт вплотную к нему, хотя место
     * рядом есть. Но отступ — вежливость, а не требование, и берётся он ровно
     * из того места, которое объект оставляет в окне.
     *
     * Объект во весь размер окна не оставляет ничего, и тогда отступа нет:
     * иначе страница вставала бы мимо края на ширину зазора. Границы у обоих
     * направлений и держат этот баланс — доехать не меньше, чем нужно, что бы
     * объект попал в окно целиком, и не больше, чем нужно, что бы он из него
     * не вышел другим краем.
     */
    if (boxStart < viewStart) return Math.max(behind - pad.start, ahead);
    if (boxEnd > viewEnd) return Math.min(ahead + pad.end, behind);
