  ...
  const sliderContent = React.useMemo(() => {
    ...
    return Array.from({ length: objLengthPerSize }, (_, index) => (
      <div
        ...
        /*
         * Нажатие — механика меню, и живёт оно там. У слайдера полосу тянут:
         * два способа сделать одно и то же спорят и на вид — курсор обещал бы
         * нажатие на том, что берут и ведут.
         *
         * Протаскиванию нажатие не мешает: браузер шлёт `click` общему предку
         * точек начала и конца, так что пронос по нескольким точкам сюда не
         * попадает, а жест в пределах одной — это и есть нажатие.
         */
        {...(mode === "sliderMenu" && {
