...
test.describe("objects.size: each (real browser)", () => {
   ...
   * чем это и отличается от заполнения.
   *
   * А 3 остаётся на 90, хотя над ней у 1 пусто: она стоит впритык к 0 поперёк,
   * и подъём выше отнял бы у них зазор. Числа посчитаны вручную.
   */
  test("объекты поднимаются в пустоту под низкими соседями по строке", async ({
    ...
    // никто ни с кем не сошёлся ближе зазора
    for (let i = 0; i < all.length; i++)
      for (let j = i + 1; j < all.length; j++) {
        const one = all[i];
        const two = all[j];
        const byX =
          Math.min(one.x + one.w, two.x + two.w) - Math.max(one.x, two.x);
        const byY =
          Math.min(one.y + one.h, two.y + two.h) - Math.max(one.y, two.y);

        expect(byX <= -10 || byY <= -10).toBe(true);
      }
