...
test.describe("objects.size: each (real browser)", () => {
  ...
  /*
   * Те же размеры, что у кладки, но линии просят строками: порядок становится
   * строгим, и под низкой карточкой остаётся ступенька — 2 уходит на 90, под
   * самую высокую в строке, а не на 50, куда её положила бы кладка.
   */
  test("objects.direction поперёк прокрутки меняет кладку на поток", async ({
    page,
  }) => {
    await page.goto("/?scenario=flowByDirection");
    await settled(page);
  ...
  /*
   * Обе стороны за объектом — по умолчанию заполнение; названное направление
   * важнее, и строки возвращаются вместе с порядком. Размеры те же, что у
   * fillFree: там 2 уезжала на 50, здесь ждёт конца строки.
   */
  test("objects.direction возвращает строки там, где по умолчанию заполнение", async ({
    page,
  }) => {
    await page.goto("/?scenario=rowsOverFill");
    await settled(page);
