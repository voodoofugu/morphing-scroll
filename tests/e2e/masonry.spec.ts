...
test.describe("objects.size: each (real browser)", () => {
  ...
  /*
   * Заполнению нужна граница поперёк, и при hybrid её взять неоткуда. А вот
   * кладке граница не нужна — ей нужен счёт колонок, и счёт как раз назван.
   * Дырок она не оставляет: карточка 2 встаёт под 0 на 50, а не под самую
   * высокую строку, как встала бы в потоке.
   */
  test("hybrid со счётом кладёт в колонки, не оставляя дыр под низкими", async ({
    page,
  }) => {
    await page.goto("/?scenario=masonryHybrid");
    await settled(page);
  ...
  /*
   * Свободное место вдоль прокрутки закрывается: 2 уходит под низкую 0, 4 под
   * 2, 5 под 3 — каждый на своё, а не строкой. При этом никто не обгоняет
   * предыдущего: 3 могла бы подняться на 40, под низкую 1, но 2 стоит на 90,
   * и выше неё нельзя. Числа посчитаны вручную.
   */
  test("объекты поднимаются в пустоту, но не выше предыдущего", async ({
    page,
  }) => {
    ...
    // сверху вниз читается по порядку: ни один не встал выше предыдущего
    const tops = all.map((b) => b.y);
    expect(tops).toEqual([...tops].sort((one, two) => one - two));
   ...
   */
  test("column отдаёт первой колонке первые ceil(n / колонок) объектов", async ({
    page,
  }) => {
    await page.goto("/?scenario=columnOrder");
    await settled(page);
  ...
  test("column в потоке собирает строки из столбцов", async ({ page }) => {
    await page.goto("/?scenario=columnOrderFlow");
    await settled(page);
